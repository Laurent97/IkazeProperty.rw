...
import { ArrowLeft, MapPin, User, Car, Calendar, Fuel, Settings, Gauge } from 'lucide-react'
...
type CarListing = Database['public']['Tables']['listings']['Row'] & {
  seller?: {
    id: string
...
export default function CarListingDetailPage() {
  ...
  const fetchListing = async () => {
    try {
      ...
      setListing(listingData as CarListing)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading car listing...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center">
            <Button
              ...
              onClick={() => {
                ...
                if (window.history.length > 1) {
                  ...
                  router.push(`/${params.locale}/listings/cars`)
                }
              }}
              className="mr-4"
            >
            ...
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-900">Car Listing Details</h1>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardContent className="p-0">
                ...
                {/* Image Gallery */}
                {allImages.length > 1 && Array.isArray(allImages) && (
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm">Year: {listing.car_details?.year_manufacture || 'N/A'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Fuel className="h-4 w-4 text-gray-500" />
                        <span className="text-sm">Fuel: {listing.car_details?.fuel_type || 'N/A'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Settings className="h-4 w-4 text-gray-500" />
                        <span className="text-sm">Transmission: {listing.car_details?.transmission || 'N/A'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Gauge className="h-4 w-4 text-gray-500" />
                        <span className="text-sm">Mileage: {listing.car_details?.mileage ? listing.car_details.mileage.toLocaleString() : 'N/A'} km</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">Engine: {listing.car_details?.engine_capacity || 'N/A'} cc</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">Color: {listing.car_details?.color || 'N/A'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">Doors: {listing.car_details?.doors || 'N/A'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">Seats: {listing.car_details?.seats || 'N/A'}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">Condition: {listing.car_details?.condition || 'N/A'}</span>
                      </div>
                    </div>
                    
                    {/* Features */}
                      {listing.car_details.features && Array.isArray(listing.car_details.features) && listing.car_details.features.length > 0 && (
                        <div className="mt-4">
                          <h4 className="font-medium mb-2">Features</h4>
                          <div className="flex flex-wrap gap-2">
                            {listing.car_details.features.map((feature: any, index: number) => (
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="space-y-2">
                    <InquiryButton listingId={listing.id} sellerId={listing.seller_id} title={listing.title} />
                  </div>
                </CardContent>
              </Card>
            )}
