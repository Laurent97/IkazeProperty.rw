...
async function GET(request) {
    try {
        ...
        const { data: rotations, error: rotationError } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabaseAdmin"].from('ad_rotations').select('ad_campaign_id').eq('is_active', true)// Note: You'll need to add placement_id to your ad_rotations table or use a different approach
        .limit(10);
        if (rotationError) {
            console.error('Error fetching rotations:', rotationError);
        }
        // For now, let's fetch all active campaigns and filter by type
        const { data: campaigns, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabaseAdmin"].from('ad_campaigns').select('*').eq('status', status).order('created_at', {
            ascending: false
        }).limit(5);
        if (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a01b9e10._.js.map